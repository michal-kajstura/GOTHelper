// import * as sweet from '../../../../node_modules/sweetalert'
// import fire from '../../../../node_modules/sweetalert2/src/sweetalert2.js';
...
function writeList() {
    ...
    path.forEach(function (line) {
        let div = $('<div>').addClass('line');
        let point1 = line['startPoint'];
        let point2 = line['endPoint'];
        let distance = geoDistance(point1['lat'], point1['lon'], point2['lat'], point2['lon']);
        distance = Number((distance / 1000).toFixed(2));
        let p = $('<p>').text(distance + ' km').addClass('distance');
        let ul = $('<ul>').addClass('outerlist');
        ul.append($('<li>').html('od: ' + `<b>${point1.name}</b>`));
        ul.append($('<li>').html('do: ' + `<b>${point2.name}</b>`));
        div.append(ul);
        div.append(p);
        $('#path').append(div);
    });
...
$(document).ready(function () {
    ...
    $('#saveTrip').click(function () {
        ...
        let jsoned = JSON.stringify(tripData);
        $.ajax({
